  ...
  <div class="student-manage-container">
    ...
    <el-card class="filter-card" shadow="never">
      <el-form :inline="true" class="filter-form">
        <el-form-item label="小组筛选" prop="stu_group_info_id">
          <el-select v-model="filterForm.stu_group_info_id" placeholder="请选择小组" clearable @change="onGroupChange" style="width: 220px;">
            <el-option v-for="item in groupList" :key="item.id" :label="item.name + item.description" :value="item.id" />
          </el-select>
        </el-form-item>
        <el-form-item>
    ...
    <el-card class="filter-card" shadow="never">
      <div class="score-filter-bar">
        <el-button type="primary" plain @click="openScoreOperationDialog" :disabled="!selectedWeek">积分操作</el-button>
        <el-button type="primary" plain @click="toggleDetailMode">{{ detailMode ? '简要信息' : '详细信息' }}</el-button>
      </div>
    </el-card>
      ...
      <el-table :data="studentList" border stripe v-loading="loading">
        ...
        <el-table-column label="操作" width="320" fixed="right">
          <template #default="{ row }">
            <el-button type="primary" size="small" @click="openScoreOperationDialog(row)">积分操作</el-button>
            ...
            <el-button type="success" size="small" @click="openScoreRecordDialog(row)">查看积分记录</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
    ...
    <StudentFormDialog
      ...
      :school-id="currentClass?.stu_school_id"
      :group-list="groupList"
      :existing-students="originalStudents"
      @success="reloadStudents"
    />

    <GroupNamingDialog
      v-model="groupNamingVisible"
      :groups="groupList"
      :class-id="currentClass?.id"
      @success="reloadStudents"
    />

    <ScoreOperationDialog
      v-model="scoreOperationVisible"
      :class-id="currentClass?.id"
      :grade="selectedGrade"
...
const openScoreRecordDialog = (row: any) => {
  currentStudentId.value = row.id
  currentStudentName.value = row.name
  scoreRecordVisible.value = true
}
...
// 导入导出（保持不变，可提取为单独模块，此处保留简化版）
const importExcel = () => { /* 原实现 */ }
const exportToExcel = () => { /* 原实现 */ }
const downloadTemplate = () => { /* 原实现 */ }
...
<style scoped>
.student-manage-container { padding: 20px; background: #f0f2f5; min-height: calc(100vh - 60px); }
.filter-card { margin-bottom: 20px; }
.filter-form { display: flex; flex-wrap: wrap; align-items: center; gap: 12px; }
.table-card { overflow-x: auto; }
.score-filter-bar { display: flex; gap: 12px; align-items: center; flex-wrap: wrap; margin-bottom: 16px; }
</style>