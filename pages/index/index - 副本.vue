<template>
	<div class="app-container">
		<el-card class="header-card" shadow="never">
			...
			<div class="week-bar">
				<span>当前周次：</span>
				<el-input-number v-model="currentWeek" :min="1" :max="20" size="small" style="width:120px"
					@change="onWeekChange" />
				<span class="tip">{{ schoolInfo }}</span>
...
<script setup lang="ts">
	...
	import { getGroups, updateGroupScore, getClasses, getTeacherInfo, tokenLogin } from '@/api/request'

	// 学校信息
	const schoolInfo = ref('延安培文学校小学部501班')
	...
	// 更新小组合计行（基于每日分数，只累加存在的成员，若全部为 null 则合计保持 null）
	...
	// 标准化小组数据：确保 days 按周一至周五顺序，每天 scores 长度为7，并重新计算 summary
	...
	const checkLogin = async () => {
		...
		try {
			...
			if (res.code === 1) {
				...
				if (userInfo.school) schoolInfo.value = userInfo.school.name+userInfo.class.name
	...
	const fetchDataFromAPI = async () => {
	  ...
	  try {
	    ...
	    if (teacherRes.code === 1 && teacherRes.data.school) {
	      schoolInfo.value = teacherRes.data.school.name
	    }
	    const classRes = await getClasses()
	    if (classRes.code !== 1 || !classRes.data.length) {
	      throw new Error('暂无班级数据')
	    }
	    const firstClass = classRes.data[0]
	    const groupsRes = await getGroups(firstClass.id)
	    if (groupsRes.code !== 1) {
	      throw new Error(groupsRes.msg || '获取小组数据失败')
	    }
	
	    // 辅助函数：将任意值转换为数字或 null
	...
	const buildTableFromGroups = () => {
	  ...
	  groupsStore.value.forEach(group => {
	    const groupRows = []
	    // 添加每天数据（确保 days 存在且每个 day 有 scores）
	    ...
	    // 添加合计行（确保 summary.totals 存在且长度足够）
...
const exportToExcel = () => {
  ...
  try {
	...
	const onWeekChange = () => {
		ElMessage.info(`切换到第${currentWeek.value}周，如需不同周数据请导入对应周Excel`)
	}
...
<style scoped>
	/* 样式与原文件相同，此处省略以节省篇幅，请直接复制原文件的样式内容 */
	.app-container { max-width: 1400px; margin: 0 auto; padding: 20px; }
	.header-card { margin-bottom: 24px; }
	.header-content { display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 16px; margin-bottom: 16px; }
	.title { font-size: 1.4rem; font-weight: 600; color: #1e293b; }
	.actions { display: flex; gap: 8px; flex-wrap: wrap; }
	.week-bar { display: flex; align-items: center; gap: 12px; flex-wrap: wrap; padding-top: 8px; border-top: 1px solid #eef2f6; }
	.tip { margin-left: auto; font-size: 0.8rem; color: #6c757d; }
	.table-card { overflow-x: auto; }
	.total-score { font-weight: bold; color: #e6a23c; }
	.table-title { text-align: center; margin-bottom: 20px; padding-bottom: 10px; border-bottom: 2px solid #eef2f6; }
	.table-title h2 { margin: 0; font-size: 1.5rem; font-weight: 600; color: #1e293b; }
	.group-name-cell { cursor: pointer; color: #409eff; font-weight: 500; }
	.group-name-cell:hover { text-decoration: underline; }
	.clickable-score { cursor: pointer; display: inline-block; padding: 4px 8px; border-radius: 4px; transition: background 0.2s; }
	.clickable-score:hover { background-color: #ecf5ff; }
	.null-score { color: #aaa; font-style: italic; cursor: pointer; }
	.null-score:hover { background-color: #f0f9eb; }
	.score-buttons { display: flex; flex-wrap: wrap; gap: 8px; justify-content: center; }
