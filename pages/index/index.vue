...
<script setup lang="ts">
	...
	import { getGroups, updateGroupScore, getClasses, getTeacherInfo, tokenLogin, addNewWeek as AddNewWeek } from '@/api/request'
	...
	const fetchDataFromAPI = async () => {
		...
		try {
			...
			const groupsRes = await getGroups(currentClass.id, groupsStore.value.week || 0)
			if (groupsRes.code !== 1) throw new Error(groupsRes.msg || '获取小组数据失败')
			console.log(groupsRes)
			currentWeek.value = groupsRes.data[0].week
	...
	const addNewWeek = async () => {
		try {
			currentWeek.value += 1
			await ElMessageBox.confirm('添加新一周将清空当前所有分数（保留不存在的成员x），是否继续？', '提示', {
			...
			for (const group of groupsStore.value) {
				...
				console.log(payload)
				await AddNewWeek(classInfo.value.id, currentWeek.value, group.name, payload)
				// 重新标准化并计算合计
				normalizeGroupData(group)
			}
			buildTableFromGroups()
			...
			if (error !== 'cancel') console.error('添加新一周失败', error)
		}
	}
...
// 导入Excel
	...
	// 其余函数（buildTableFromGroups, spanMethod, getRankType, handleGroupClick, confirmGroupChange, openStudentScoreEditor, confirmPersonalChange, onMounted等）保持原有不变
	// 由于原文件已包含完整实现，此处仅示意，实际替换时请保留原有函数体
