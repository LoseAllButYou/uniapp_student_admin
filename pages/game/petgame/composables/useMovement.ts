...
export function useMovement(sprites: Ref<Sprite[]>) {
	...
	const buildSpriteData = (pets: any[], existingSprites: Sprite[]) => {
		...
		let petIdx = 0
		console.log('buildSpriteData - 原始pets数据:', pets)
		const result = pets.map((p: any) => {
			...
			const spriteData = {
				...
				lastFeedAt: p.last_feed_at || null,
				lastInteractAt: p.last_interact_at || null,
				x: initX,
				y: initY,
			...
			console.log(`buildSpriteData - group_id=${p.group_id}, has_pet=${p.has_pet}, p.id=${p.id}, petId=${spriteData.petId}`)
			return spriteData
		})
		console.log('buildSpriteData - 最终sprites:', result)
		return result
	}
